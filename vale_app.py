...
def generate_report(image_tensor, clinical_info):
    try:
        # Handle empty clinical information
        if clinical_info.strip():
            # Tokenize clinical information if provided
            clinical_tokens = tokenizer(clinical_info, return_tensors="pt", padding=True, truncation=True).to(device)
            input_ids = clinical_tokens.input_ids
            attention_mask = clinical_tokens.attention_mask
        else:
            # Use None for input_ids and attention_mask when clinical_info is empty
            input_ids = None
            attention_mask = None
...
clinical_info = st.text_area("Enter Clinical Information (optional, e.g., patient history, symptoms)", "")
...
if uploaded_file:
    try:
        ...
        report = generate_report(preprocessed_image, clinical_info)
